...
function pms_add_plugin_notification_new_add_on() {
	...
	// $notification_id = 'pms_new_add_on_labels_edit';
	// $message = '<img style="float: left; margin: 10px 12px 10px 0; max-width: 80px;" src="' . PMS_PLUGIN_DIR_URL . 'assets/images/pms-add-on-labels-edit-logo.png" />';
	// $message .= '<p style="margin-top: 16px;">' . __( 'Check out the new <strong>Labels Edit</strong> add-on, which gives you the possibility to edit any string that is coming from the <strong>Paid Member Subscriptions</strong> plugin.', 'paid-member-subscriptions' ) . '</p>';
	// $message .= '<p><a href="https://www.cozmoslabs.com/add-ons/pms-labels-edit/?utm_source=wp-backend&utm_medium=addon-notification&utm_campaign=PMSFree" class="button-primary">' . __( 'More information', 'paid-member-subscriptions' ) . '</a></p>';
