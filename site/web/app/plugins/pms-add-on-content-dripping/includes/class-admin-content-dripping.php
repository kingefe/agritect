...
Class PMS_Content_Dripping_Custom_Post_Type extends PMS_Custom_Post_Type {
    ...
    public function register_custom_statuses() {
        ...
        register_post_status( 'active', array(
            'label'                     => _x( 'Active', 'Active status for content dripping set.', 'pms-content-dripping-add-on' ),
            'public'                    => true,
            'exclude_from_search'       => false,
            'show_in_admin_all_list'    => true,
            'show_in_admin_status_list' => true,
            'label_count'               => _n_noop( 'Active <span class="count">(%s)</span>', 'Active <span class="count">(%s)</span>', 'pms-content-dripping-add-on' )
        ));
        register_post_status( 'inactive', array(
            'label'                     => _x( 'Inactive', 'Inactive status for content dripping set.', 'pms-content-dripping-add-on' ),
            'public'                    => true,
            'exclude_from_search'       => false,
            'show_in_admin_all_list'    => true,
            'show_in_admin_status_list' => true,
            'label_count'               => _n_noop( 'Inactive <span class="count">(%s)</span>', 'Inactive <span class="count">(%s)</span>', 'pms-content-dripping-add-on' )
        ));
    ...
    public function add_post_row_actions( $actions, $post ) {
        ...
        if( $post->post_status == 'active' )
            $activate_deactivate = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'pms-action' => 'deactivate_content_dripping_set', 'post_id' => $post->ID ) ), 'pms_content_dripping_set_nonce' ) ) . '">' . __( 'Deactivate', 'pms-content-dripping-add-on' ) . '</a>';
        else
            $activate_deactivate = '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'pms-action' => 'activate_content_dripping_set', 'post_id' => $post->ID ) ), 'pms_content_dripping_set_nonce' ) ) . '">' . __( 'Activate', 'pms-content-dripping-add-on' ) . '</a>';
    ...
    public static function manage_posts_columns( $columns ) {
        ...
        $new_columns = array_merge( $columns, array(
            'subscription_plan' => __( 'Subscription Plan', 'pms-content-dripping-add-on' ),
            'shortcode'         => __( 'Contents Table Shortcode', 'pms-content-dripping-add-on' ),
            'status'            => __( 'Status', 'pms-content-dripping-add-on' )
        ));
    ...
    public static function manage_posts_custom_column( $column, $post_id ) {
        ...
        if( $column == 'status' ) {
            ...
            if( $status == 'active' )
                echo $subscription_plan_status_dot . '<span>' . __( 'Active', 'pms-content-dripping-add-on' ) . '</span>';
            else
                echo $subscription_plan_status_dot . '<span>' . __( 'Inactive', 'pms-content-dripping-add-on' ) . '</span>';
        }
    ...
    function set_custom_messages( $messages ) {
        ...
        $messages[$this->post_type] = array(
            0  => 	'',
            1  => 	__( 'Content Drip Set updated.', 'pms-content-dripping-add-on' ),
            2  => 	__( 'Custom field updated.', 'pms-content-dripping-add-on' ),
            3  => 	__( 'Custom field deleted.', 'pms-content-dripping-add-on' ),
            4  => 	__( 'Content Drip Set updated.', 'pms-content-dripping-add-on' ),
            5  => 	isset( $_GET['revision'] ) ? sprintf( __( 'Content Drip Set' . ' restored to revision from %s', 'pms-content-dripping-add-on' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
            6  => 	__( 'Content Drip Set saved.', 'pms-content-dripping-add-on' ),
            7  => 	__( 'Content Drip Set saved.', 'pms-content-dripping-add-on' ),
            8  => 	__( 'Content Drip Set submitted.', 'pms-content-dripping-add-on' ),
            9  => 	sprintf( __( 'Content Drip Set' . ' scheduled for: <strong>%1$s</strong>.', 'pms-content-dripping-add-on' ), date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ) ),
            10 =>	__( 'Content Drip Set draft updated.', 'pms-content-dripping-add-on' ),
        );
...
$pms_cpt_content_dripping = new PMS_Content_Dripping_Custom_Post_Type( 'pms-content-dripping', __( 'Content Drip Set', 'pms-content-dripping-add-on' ), __( 'Content Drip Sets', 'pms-content-dripping-add-on' ), $args );
