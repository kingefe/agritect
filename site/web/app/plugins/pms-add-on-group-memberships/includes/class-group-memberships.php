...
Class PMS_Group_Memberships {
    ...
    private function get_members_row_actions( $reference, $subscription_id ){
        ...
        if( !is_numeric( $reference) || !pms_gm_is_group_owner( $reference ) )
            $actions = '<a class="remove" data-reference="'.$reference.'" data-subscription="'.$subscription_id.'" href="#">'. esc_html__( 'Remove', 'paid-member-subscriptions' ) .'</a>';
        else
            $actions = '';

        if( !is_numeric( $reference ) )
            $actions .= '<a class="resend" data-reference="'.$reference.'" data-subscription="'.$subscription_id.'" href="#">'. esc_html__( 'Resend Invite', 'paid-member-subscriptions' ) .'</a>';
