...
if( class_exists('PMS_Meta_Box') ) {
    Class PMS_Meta_Box_Subscription_Content_Restriction extends PMS_Meta_Box
    {
        ...
        static function pms_output_content_restriction_row($subscription_plan, $nr_of_rules, $post = NULL, $current_nr_of_rules = NULL)
        {
            ...
            for ($i = 1; $i <= $nr_of_rules; $i++) {
                ...
                $output .= '<label for="pms_content_rule_post_type[]" class="pms-meta-box-field-label">' . __('Post Type', 'pmstxt') . '</label>';
                $output .= '<select name="pms_content_rule_post_type[]" class="pms-post-type-select">';
                $output .= '<option value=""> ' . __('Choose...', 'pmstxt') . '</option>';
                ...
                $output .= '</select>';
                $output .= '<p class="description">' . __('Choose a post type.', 'pmstxt') . '</p>';
                $output .= '</div>';
                ...
                if ($selected_post_type == '')
                    ...
                    $disabled = '';
                $output .= '<a href="#" class="pms-add-taxonomy' . $disabled . '" data-current-post-type="' . esc_attr( $selected_post_type ) . '" data-nr-of-rules="' . ((int)$current_counter) . '"><span class="pms_taxonomy-line"><!-- --></span><span class="dashicons dashicons-plus"></span>' . __('Add Taxonomy', 'pmstxt') . '</a>';

                $output .= '</div>';
                $output .= '<div class="pms_-remove-rule"><a href="#" title="' . __('Remove this rule', 'pmstxt') . '"><span class="dashicons dashicons-no"></span></a></div>';
                $output .= '</div>';
            }

            $output .= '<div id="pms_add-new-rule-container"><a href="#" class="pms-add-rule" data-post-id="' . esc_attr( $post->ID ) . '" data-nr-of-rules="' . ((int)$current_counter + 1) . '"><span class="dashicons dashicons-plus"></span>' . __('Add Rule', 'pmstxt') . '</a></div>';
        ...
        static function pms_render_taxonomy_select($taxonomies_for_post_type, $current_rule_counter, $current_taxonomy = null)
        {
            ...
            $output .= '<label for="pms_content_rule_taxonomy_' . esc_attr( $current_rule_counter ) . '[]" class="pms-meta-box-field-label pms-field-label-taxonomy"><span class="dashicons dashicons-editor-break"></span>' . __('Taxonomy', 'pmstxt') . '</label>';
            ...
            $output .= '</select>';
            $output .= '<a href="#" class="pms-remove-taxonomy">' . __('Remove', 'pmstxt') . '</a>';
    ...
    $pms_meta_box_subscription_content_restriction = new PMS_Meta_Box_Subscription_Content_Restriction('pms_subscription_content_restriction', __('Global Content Restriction', 'pmstxt'), 'pms-subscription', 'normal');
