...
function pms_inv_payment_complete_generate_invoice_number( $payment_id = 0, $data = array() ) {
    ...
    // Make sure the invoice number is not duplicated
    if( !pms_inv_check_if_payment_meta_exists( 'pms_inv_invoice_number', $invoice_number ) ){
        // Add the invoice number to the payment
        pms_add_payment_meta( $payment_id, 'pms_inv_invoice_number', $invoice_number, true );
    } else {
        update_option( 'pms_inv_invoice_number', $invoice_number + 2 );
        pms_add_payment_meta( $payment_id, 'pms_inv_invoice_number', $invoice_number + 1, true );
    }
}
